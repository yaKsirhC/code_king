...
export default function QuestionAnswer({question, answer}: {question: string, answer: string}) {
	...
	createEffect(() => {
		if(show() === false) {
			(answerRef as HTMLDivElement).style.transform = 'translateY(-100%)';
			(answerRef as HTMLDivElement).style.opacity = '0';
			(hrRef as HTMLElement).style.top = '0';
			
		}else {
			(answerRef as HTMLDivElement).style.transform = '';
  ...
  return (
	<div class="">
		...
		<div class="overflow-hidden relative">
			<div ref={answerRef as any} class="text-gray-800 transition-all answer">
				<h2 class="max-w-50ch">
					{answer}
				</h2>
			</div>
